...
def inference(parameters, names, verbose=True):
    """
    ...
    with tf.Graph().as_default():
        ...
        with tf.Session(config=session_config) as session:
            ...
            # load input images
            datum_l_cc = utils.load_images(parameters['image_path'], names[0])
            datum_r_cc = utils.load_images(parameters['image_path'], names[1])
            datum_l_mlo = utils.load_images(parameters['image_path'], names[2])
            datum_r_mlo = utils.load_images(parameters['image_path'], names[3])
